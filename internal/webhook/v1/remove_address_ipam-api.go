...
func RemoveAddressIpamAPI(ipAddress string, ipFamily string, annotations map[string]string, service *corev1.Service, secret *corev1.Secret, clusterId string, namespaceId string) (apicontracts.IpamApiResponse, error) {
	...
	responseAddrObject, err = utils.DeleteIP(requestAddrObject)
	if err != nil {
		return apicontracts.IpamApiResponse{}, fmt.Errorf("failed to remove IP for address-family %s for Service %s: %w", ipFamily, service.GetName(), err)
	}
