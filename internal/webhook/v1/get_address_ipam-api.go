...
func GetAddressIpamAPI(ipFamily string, annotations map[string]string, service *corev1.Service, secret *corev1.Secret, clusterId string, namespaceId string) (apicontracts.IpamApiResponse, error) {
	...
	// Create request object for IPAM API
	...
	// Request IP from IPAM API
	var responseAddrObject apicontracts.IpamApiResponse
	responseAddrObject, err = utils.RequestIP(requestAddrObject)
	if err != nil {
		return apicontracts.IpamApiResponse{}, fmt.Errorf("failed to request IP for address-family %s for Service %s: %w", ipFamily, service.GetName(), err)
	}
